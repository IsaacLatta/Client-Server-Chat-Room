...
private:
	...
	void login()
	{
		...
		client.set_encryption_keys(key_pair.first, key_pair.second);

		if (!client.recv_server_pk())
		{
			throw std::runtime_error("[-] Failed to receive server public key");
		}
