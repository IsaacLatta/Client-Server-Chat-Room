	...
	void fileTransfer(std::string& message, User& user)
	{
		...
		try
		{
			...
			if (recipUser->awaitDecision())
			{
				...
				// Server is downloading/uploading
				if (user.getSocket() == server.getListenSocket() || recipUser->getSocket() == server.getListenSocket())
				{
					server.setFile(fileName);
				}

				server.transferFile(user.getSocket(), recipUser->getSocket()); // Transfer the file
		...
		while (!shouldQuit)
		{
			...
			while (true)
			{
				...
				if (server.userExists(user_name))
				{
					...
					server.sendMessage(msg_to_send, clientSock);
				}
				else
				{
					msg_to_send = "$Valid";
