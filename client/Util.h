...
namespace util 
{
	...
	template <typename T> 
	T vectorToData(std::vector<unsigned char>& v)
	{
		static_assert(std::is_trivially_copyable<T>::value, "T must be trivially copyable");
		static_assert(!(std::is_pointer<T>::value), "T must not be a pointer");
		if (v.size() != sizeof(T))
		{
			throw std::length_error("[-] Size mismatch on data conversion");
		}

		T ret_data;
		std::memcpy(&ret_data, v.data(), sizeof(T));
	...
	std::vector<unsigned char> strToBin_IP(const std::string& ip) {
		...
		{
			binaryIP.push_back(static_cast<unsigned char>(htons(std::stoi(ipPart))));
		}
	...
	std::string binToStr_IP(std::vector<unsigned char>& bin_IP)
	{
		...
		{
			IP += std::to_string(ntohs(bin_IP[i]));
			if (i < bin_IP.size() - 1)
			{
				IP += ".";
			}
